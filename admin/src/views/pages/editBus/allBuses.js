...
const ShowBuses = () => {
    ...
    try {
      ...
      if (response.data === "Bus updated successfully") {
        setBuses((prevBuses) =>
          prevBuses.map((bus) =>
            bus.id === editedBus.id
              ? { ...bus, ...editedBusDetails, driver: drivers.find(d => d.id === editedBusDetails.driverId) || null }
              : bus
          )
        );
  ...
  const fetchBuses = async () => {
    try {
      const busResponse = await axiosInst.get("/bus/getallbuses", {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
        },
      });
  
      const driverResponse = await axiosInst.get("/drivers/all", {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
        },
      });
  
      const driversMap = new Map(driverResponse.data.map(driver => [driver.id, driver]));
  
      const updatedBuses = busResponse.data.map(bus => ({
        ...bus,
        driver: driversMap.get(bus.driverId) || null,
      }));
  
      setBuses(updatedBuses);
      setDrivers(driverResponse.data);
    } catch (error) {
      console.error("Error fetching buses or drivers:", error);
      toast.error("Failed to fetch buses or drivers.");
    }
  };
  ...
  const handleDownload = () => {
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      pdf.text(
        "This is a system-generated PDF",
        pdf.internal.pageSize.width / 2,
          ...
          <tbody>
            {buses.map((bus) => (
              <tr key={bus.id}>
                ...
                <td>
                  {bus.driver
                    ? `${bus.driver.firstName} ${bus.driver.lastName} (${bus.driver.licenseNumber})`
                    : "Not Assigned"}
                </td> {/* Отображение водителя */}
                <td>
                  <FontAwesomeIcon
                    icon={faEdit}
                    color="blue"
          ...
          <Form>
            ...
            <Form.Group>
              ...
              <Form.Control
                ...
                onChange={(e) => setEditedBusDetails({ ...editedBusDetails, driverId: e.target.value })}
              >
